...
const featuredServices = [
  ...
  },
  {
    ...
    verified: true,
    image: 'https://picsum.photos/seed/dj-featured/600/400',
  ...
  },
  {
    ...
    verified: true,
    image: 'https://picsum.photos/seed/photography-featured/600/400',
  ...
  },
  {
    ...
    verified: true,
    image: 'https://picsum.photos/seed/venue-featured/600/400',
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-14">
          <div>
            ...
            <h2 className="text-3xl lg:text-5xl font-bold text-foreground tracking-tight text-balance">Profissionais em Alta</h2>
          </div>
          <Link href="/explorar">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {featuredServices.map((service) => (
            <Link key={service.id} href={`/servico/${service.id}`} className="group bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl overflow-hidden card-hover">
              <div className="relative aspect-[4/3] overflow-hidden bg-muted">
                ...
                <div className="absolute top-4 left-4">
                  <Badge className="bg-background/90 text-foreground backdrop-blur-sm border-0 font-medium">{service.category}</Badge>
                </div>
                {service.verified && (
                  <div className="absolute top-4 right-4">
                    <div className="bg-primary text-primary-foreground rounded-full p-1.5 shadow-lg"><CheckCircle className="w-4 h-4" /></div>
                  </div>
                )}
              </div>
              <div className="p-6">
                ...
                <div className="flex items-center gap-1.5 text-sm text-muted-foreground mb-4">
                  <MapPin className="w-4 h-4 shrink-0" /><span>{service.location}</span>
                </div>
                <div className="flex flex-wrap gap-2 mb-5">
                  {service.tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="text-xs bg-muted/50 text-muted-foreground px-2.5 py-1 rounded-full">{tag}</span>
                  ))}
                </div>
